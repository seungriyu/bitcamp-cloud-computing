...
public class MemberService {
    ...
    //메서드에 @Transactional 애노테이션을 붙이면, 메서드에서 수행하는 작업들을 한 단위로 묶어서 다루겠다는 의미다.
    //따라서 작업들 중에 한 개의 작업이라도 실패하면, 이전에 성공했던 모든 작업들이 취소된다.
    //메서드 호출이 예외없이 정상적으로 끝나면, DBMS에 commit을 요청하여 지금까지 한 작업을 실제 테이블에 적용시킨다.
