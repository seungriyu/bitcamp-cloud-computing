-- 수강신청
CREATE TABLE MY_SCHEMA.P1_APPL (
...
-- 학생
CREATE TABLE MY_SCHEMA.P1_STUD (
	SNO      INTEGER      NOT NULL COMMENT '학생번호', -- 학생번호
...
-- 학생 유니크 인덱스
CREATE UNIQUE INDEX UIX_P1_STUD
	ON MY_SCHEMA.P1_STUD ( -- 학생
	);

-- 강사
CREATE TABLE MY_SCHEMA.P1_TCH (
...
CREATE UNIQUE INDEX UIX_P1_TCH
	ON MY_SCHEMA.P1_TCH ( -- 강사
	);

-- 강의실
CREATE TABLE MY_SCHEMA.P1_CLS_ROOM (
...
-- 강의실
ALTER TABLE MY_SCHEMA.P1_CLS_ROOM
...
CREATE UNIQUE INDEX UIX_P1_MGR
	ON MY_SCHEMA.P1_MGR ( -- 매니저
	);

-- 개설교육과정
CREATE TABLE MY_SCHEMA.P1_OPEN_CORS (
	OCNO   INTEGER     NOT NULL COMMENT '개설교육과정번호', -- 개설교육과정번호
...
-- 개설교육과정
ALTER TABLE MY_SCHEMA.P1_OPEN_CORS
...
-- 강의과목
CREATE TABLE MY_SCHEMA.P1_TCH_SUB (
	SNO INTEGER NOT NULL COMMENT '과목번호', -- 과목번호
	MNO INTEGER NOT NULL COMMENT '강사번호' -- 강사번호
)
COMMENT '강의과목';
...
-- 강의과목
ALTER TABLE MY_SCHEMA.P1_TCH_SUB
...
-- 프로젝트참여자
CREATE TABLE MY_SCHEMA.P1_PRJ_MEMB (
	PNO INTEGER NOT NULL COMMENT '프로젝트번호', -- 프로젝트번호
	SNO INTEGER NOT NULL COMMENT '학생번호' -- 학생번호
)
COMMENT '프로젝트참여자';
...
-- 프로젝트참여자
ALTER TABLE MY_SCHEMA.P1_PRJ_MEMB
...
-- 교육과정과목
CREATE TABLE MY_SCHEMA.P1_CORS_SUB (
	OCNO INTEGER NOT NULL COMMENT '개설교육과정번호', -- 개설교육과정번호
	SNO  INTEGER NOT NULL COMMENT '과목번호', -- 과목번호
...
-- 교육과정과목
ALTER TABLE MY_SCHEMA.P1_CORS_SUB
...
-- 교육과정수강생
CREATE TABLE MY_SCHEMA.P1_CORS_STUD (
	OCNO INTEGER NOT NULL COMMENT '개설교육과정번호', -- 개설교육과정번호
	SNO  INTEGER NOT NULL COMMENT '학생번호' -- 학생번호
)
COMMENT '교육과정수강생';
...
-- 교육과정수강생
ALTER TABLE MY_SCHEMA.P1_CORS_STUD
...
-- 강의실사진
CREATE TABLE MY_SCHEMA.P1_CLR_PHOT (
...
-- 강의실사진
ALTER TABLE MY_SCHEMA.P1_CLR_PHOT
...
-- 주소
CREATE TABLE MY_SCHEMA.P1_ADDR (
...
-- 교육과정
CREATE TABLE MY_SCHEMA.P1_CORS (
	CRNO INTEGER      NOT NULL COMMENT '교육과정번호', -- 교육과정번호
...
-- 회원
CREATE TABLE MY_SCHEMA.P1_MEMB (
...
ALTER TABLE MY_SCHEMA.P1_APPL
	ADD CONSTRAINT FK_P1_STUD_TO_P1_APPL -- 학생 -> 수강신청
		...
		REFERENCES MY_SCHEMA.P1_STUD ( -- 학생
			SNO -- 학생번호
		);
...
ALTER TABLE MY_SCHEMA.P1_APPL
	ADD CONSTRAINT FK_P1_OPEN_CORS_TO_P1_APPL -- 개설교육과정 -> 수강신청
		...
		REFERENCES MY_SCHEMA.P1_OPEN_CORS ( -- 개설교육과정
			OCNO -- 개설교육과정번호
		);
...
ALTER TABLE MY_SCHEMA.P1_STUD
	ADD CONSTRAINT FK_P1_ADDR_TO_P1_STUD -- 주소 -> 학생
		...
		REFERENCES MY_SCHEMA.P1_ADDR ( -- 주소
			ADRNO -- 주소번호
		);
...
ALTER TABLE MY_SCHEMA.P1_STUD
	ADD CONSTRAINT FK_P1_MEMB_TO_P1_STUD -- 회원 -> 학생
		...
		REFERENCES MY_SCHEMA.P1_MEMB ( -- 회원
			MNO -- 회원번호
		);
...
ALTER TABLE MY_SCHEMA.P1_OPEN_CORS
	ADD CONSTRAINT FK_P1_CLS_ROOM_TO_P1_OPEN_CORS -- 강의실 -> 개설교육과정
		...
		REFERENCES MY_SCHEMA.P1_CLS_ROOM ( -- 강의실
			CLRNO -- 강의실번호
		);
...
ALTER TABLE MY_SCHEMA.P1_OPEN_CORS
	ADD CONSTRAINT FK_P1_MGR_TO_P1_OPEN_CORS -- 매니저 -> 개설교육과정
		...
		REFERENCES MY_SCHEMA.P1_MGR ( -- 매니저
			MGNO -- 매니저번호
		);
...
ALTER TABLE MY_SCHEMA.P1_OPEN_CORS
	ADD CONSTRAINT FK_P1_CORS_TO_P1_OPEN_CORS -- 교육과정 -> 개설교육과정
		...
		REFERENCES MY_SCHEMA.P1_CORS ( -- 교육과정
			CRNO -- 교육과정번호
		);
...
ALTER TABLE MY_SCHEMA.P1_TCH_SUB
	ADD CONSTRAINT FK_P1_TCH_TO_P1_TCH_SUB -- 강사 -> 강의과목
		...
		REFERENCES MY_SCHEMA.P1_TCH ( -- 강사
			MNO -- 강사번호
		);
...
ALTER TABLE MY_SCHEMA.P1_TCH_SUB
	ADD CONSTRAINT FK_P1_SUB_TO_P1_TCH_SUB -- 과목 -> 강의과목
		...
		REFERENCES MY_SCHEMA.P1_SUB ( -- 과목
			SNO -- 과목번호
		);
...
ALTER TABLE MY_SCHEMA.P1_PRJ_MEMB
	ADD CONSTRAINT FK_P1_PRJ_TO_P1_PRJ_MEMB -- 프로젝트 -> 프로젝트참여자
		...
		REFERENCES MY_SCHEMA.P1_PRJ ( -- 프로젝트
			PNO -- 프로젝트번호
		);
